...
import static org.personal.beertaster.elements.Coordinates.*;
...
public class MainToolBar extends ToolBar {
  ...
  public MainToolBar(final List<Router> routers, final List<Optimiser> optimisers) {
    txtLatitude = new TextField(
        Double.toString(BreweryManager.ORIGIN.getCoordinates().getLatitude()));
    txtLongitude = new TextField(
        Double.toString(BreweryManager.ORIGIN.getCoordinates().getLongitude()));
  ...
  public void setOnRouteCallback(final BiConsumer<Coordinates, Router> callback) {
    btnRoute.setOnAction(e -> {
      ...
      final Double longitude = parse(txtLongitude.getText(), MIN_LONGITUDE, MAX_LONGITUDE,
          "Longitude");

      if (Objects.nonNull(latitude) && Objects.nonNull(longitude) && Objects
          .nonNull(routePlanner)) {
